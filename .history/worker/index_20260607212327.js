...
async function aiChat(request, env) {
  ...
  catch { return json({ error: "Invalid JSON" }, 400, CORS_NOCACHE); }
  ...
  let aiResp;
  try {
    aiResp = await env.AI.run(AI_MODEL, { messages, temperature: 0.3, max_tokens: 512 });
  } catch (err) {
    return json({ error: "ai run failed", message: err.message }, 502, CORS_NOCACHE);
  }
  ...
  try {
    ...
    parsed = jsonStart >= 0 ? JSON.parse(raw.slice(jsonStart, jsonEnd + 1)) : { reply: raw, actions: [] };
  } catch {
    parsed = { reply: raw || "(no reply)", actions: [] };
  }
  return json(
    { reply: String(parsed.reply || "").slice(0, 1000), actions: Array.isArray(parsed.actions) ? parsed.actions.slice(0, 16) : [] },
    200, CORS_NOCACHE,
  );
}

// ════════════════════════════════════════════════════════════
// Prefab — resolved character JSON for external Three.js/HTML5 games
// GET /api/prefab/:uuid → { uuid, model:{...}, equipment:{...}, anims:{...}, stats:{...} }
// ════════════════════════════════════════════════════════════
async function getPrefab(env, uuid) {
  const apiBase = (env.GRUDGE_API_BASE || "https://api.grudge-studio.com").replace(/\/$/, "");
  let character = null;
  try {
    const res = await fetch(`${apiBase}/api/characters/${encodeURIComponent(uuid)}`, {
  ...
  ).bind(character.factionId, character.raceId).first();
  if (!modelRow) return json({ error: "model not registered", factionId: character.factionId, raceId: character.raceId }, 404);

  const { results: slots } = await env.DB.prepare(
    "SELECT * FROM equipment_slots WHERE model_id = ? ORDER BY sort_order",
  ).bind(modelRow.id).all();
  ...
  for (const p of animPacks) {
    anims[p.pack_key] = {
      name: p.name, path: p.r2_base_url,
      files: JSON.parse(p.files || "[]"),
  ...
  return json({
    ...
    model: {
      factionId: modelRow.faction_id, raceId: modelRow.race_id, prefix: modelRow.prefix,
      url: modelRow.r2_url + "?" + ASSET_VERSION,
      format: modelRow.format, skeletonType: modelRow.skeleton_type,
    },
    equipment: {
      equipped: character.equipped || {},
      slots: slots.map((s) => ({ slot: s.slot, group: s.group, variants: JSON.parse(s.variants || "[]") })),
    },
    animations: anims,
